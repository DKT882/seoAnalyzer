import { validateHostnameSsrf } from '../utils/ssrfGuard.js';
import { validateAndNormalizeUrl, resolveAbsoluteUrl } from '../utils/urlUtils.js';
...
export async function safeFetch(targetUrl: string, options: FetchOptions = {}): Promise<FetchResult> {
  ...
  while (redirectCount <= maxRedirects) {
    ...
    if (!urlValidation.isValid || !urlValidation.parsedUrl) {
      throw new Error(`Invalid URL: ${urlValidation.error || 'Cannot parse URL'}`);
    }
    ...
    if (!ssrfCheck.isSafe) {
      throw new Error(`SSRF Protection Blocked: ${ssrfCheck.reason}`);
    }
    ...
    try {
      ...
      if ([301, 302, 303, 307, 308].includes(response.status)) {
        ...
        if (!resolvedRedirectUrl) {
          throw new Error(`Unsafe or unresolvable redirect destination: ${location}`);
        }

        if (redirectChain.includes(resolvedRedirectUrl)) {
          throw new Error(`Redirect loop detected: ${resolvedRedirectUrl} already in chain.`);
        }
      ...
      if (contentLengthHeader && parseInt(contentLengthHeader, 10) > maxSizeBytes) {
        clearTimeout(timer);
        throw new Error(`Page size exceeds maximum allowable limit of ${Math.round(maxSizeBytes / 1024 / 1024)}MB.`);
      }
        ...
        while (true) {
          ...
          if (value) {
            ...
            if (totalBytes > maxSizeBytes) {
              controller.abort();
              throw new Error(`Response body exceeded ${Math.round(maxSizeBytes / 1024 / 1024)}MB size limit while downloading.`);
            }
            chunks.push(value);
          }
        }
