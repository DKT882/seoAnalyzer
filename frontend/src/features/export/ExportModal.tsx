...
export function ExportModal({ report, onClose }: ExportModalProps) {
  ...
  const xlsxUrl = apiClient.getExportUrl(report.id, 'xlsx');
  const jsonUrl = apiClient.getExportUrl(report.id, 'json');
  const htmlUrl = apiClient.getExportUrl(report.id, 'html');
  const dynamicCsvUrl = apiClient.getExportUrl(report.id, 'csv', selectedCsvType);
    ...
    >
      ...
      >
        ...
        <p style={{ fontSize: '0.85rem', color: 'var(--text-secondary)', marginBottom: '1.5rem' }}>
          Choose your preferred export format for "{report.url}".
        </p>

        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.85rem' }}>
          {/* Professional 18-Sheet XLSX Workbook */}
          <a
            href={xlsxUrl}
            download
            style={{
